...
# Get build version
MAJOR=$(cat $DIR/vendor/dk/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
MINOR=$(cat $DIR/vendor/dk/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
MAINTENANCE=$(cat $DIR/vendor/dk/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
TAG=$(cat $DIR/vendor/dk/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
...
echo -e "${cya}Building ${bldcya}BOLT $VERSION for $DEVICE ${txtrst}";
...
if [ -n "${INTERACTIVE}" ]; then
        ...
        if [ "${VENDOR}" == "bolt" ]; then
                echo -e "[${bldgrn}lunch bolt_$DEVICE-userdebug${bldblu}]${txtrst}"
        else
