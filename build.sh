...
VERSION_MAJOR=$(cat $DIR_ROOT/vendor/dk/configs/version.mk | grep 'PRODUCT_VERSION_MAJOR = *' | sed 's/PRODUCT_VERSION_MAJOR = //g')
VERSION_MINOR=$(cat $DIR_ROOT/vendor/dk/configs/version.mk | grep 'PRODUCT_VERSION_MINOR = *' | sed 's/PRODUCT_VERSION_MINOR = //g')
VERSION_MAINTENANCE=$(cat $DIR_ROOT/vendor/dk/configs/version.mk | grep 'PRODUCT_VERSION_MAINTENANCE = *' | sed 's/PRODUCT_VERSION_MAINTENANCE = //g')
VERSION_TAG=$(cat $DIR_ROOT/vendor/dk/configs/version.mk | grep 'BOLT_BUILD_TYPE = *' | sed 's/BOLT_BUILD_TYPE = //g')
...
echo -e "${CLR_BLD_GRN}Building BOLT $VERSION for $DEVICE${CLR_RST}"
...
if [ "$FLAG_USER_BUILD" = 'y' ]; then
        lunch "bolt_$DEVICE-user"
else
        lunch "bolt_$DEVICE-userdebug"
fi
echo -e ""
